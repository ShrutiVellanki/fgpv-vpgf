...
function legendServiceFactory(
            ...
            function _addChildBlock(item, parentLegendGroup) {
                ...
                function _addChildItemToAPI(apiLegendElement, parentBlock) {
                    ...
                    if (parentElement && parentElement.children) {
                        ...
                        if (mApi === null) {
                            events.$on(events.rvApiMapAdded, (_, api) => {
                                _addChildItemToAPI(apiLegendElement, parentBlock);
                            });
                        } else {
    ...
    /**
     * Return the element with given parentBlock in the Legend API
     *
     ...
     * */
    function _findParentElement(parentBlock, list) {

        if (mApi === null && list === undefined) {
            events.$on(events.rvApiMapAdded, (_, api) => {
                list = mApi.ui.configLegend.children;
                return _getParentElement(parentBlock, list);
            });
        } else {
            if (list === undefined) {
                list = mApi.ui.configLegend.children;
     ...
     * @param {Array<LegendItem | LegendGroup>} list list of elements to search through
     */
    function _getParentElement(parentBlock, list) {
